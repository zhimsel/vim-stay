...
function! stay#shim#globpath(path, glob, nosuf, list) abort
  if     v:version < 700 && a:list is 1
    throw 'No List support before Vim 7!'
  elseif v:version < 702 || (v:version is 702 && !has('patch-051'))
    let l:result = globpath(a:path, a:glob)
  elseif v:version < 704 || (v:version is 704 && !has('patch-279'))
    let l:result = globpath(a:path, a:glob, a:nosuf)
  else
    let l:result = globpath(a:path, a:glob, a:nosuf, a:list)
  endif
  return a:list is 1 && type(l:result) isnot type([])
     \ ? split(l:result, '\n')
     \ : l:result
endfunction
